...
class EventsManager {
    constructor() {
        ...
        // Start cleaner
        setInterval(this._cleanOldPreviousEvents, 30000, this);
    }
    ...
    /**
     * Clean up old previous events which aren't available anymore in the GTFS-RT feed
     * @param self
     * @private
     */
    _cleanOldPreviousEvents(self) {
        // For each agency
        for(let k=0; k < Object.keys(self.previousEvents); k++) {
            let agency = Object.keys(self.previousEvents)[k];

            // Check if events are older than the current time, we can't receive events which are in the past
            for(let e=0; e < self.previousEvents[agency]; e++) {
                let departureTime = new Date(self.previousEvents[agency][e]['departureTime']);
                if(departureTime > new Date()) {
                    console.log('Removing previous events (TTL):' + self.previousEvents[agency][e]['@id']);
                    this.previousEvents[agency].splice(self.previousEvents[agency].indexOf(e), 1);
                }
            }
        }
    }
