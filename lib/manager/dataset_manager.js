...
class DatasetManager {
    ...
    async manage() {
        ...
        this._datasets.forEach(async (dataset, index) => {
            try {
                ...
                // Generate vehicle index and tree
                await this.updateVehicleIndex(dataset).then(() => {
                    this.writeVehicleIndexToDisk(dataset).then(() => {
                        this.generateVehicleIndexTree(dataset);
                    });
                })
            } catch (err) {
                logger.error(err);
    ...
    /**
     * @brief Updates the vehicle index when the static fragments are ready.
     * @author Dylan Van Assche
     */
    updateVehicleIndex(dataset) {
        return new Promise((resolve, reject) => {
            ...
            let companyName = dataset.companyName;
            let vehicle_path = this.storage + '/vehicles/' + companyName;
            if (!fs.existsSync(vehicle_path)) {
                fs.mkdirSync(vehicle_path);
            }

            //let data = [];
            this.vehicleIndexData = [];
            let path = this.storage + '/linked_pages/' + companyName;

            fs.readdir(path, async (err, sets) => {
                logger.debug('Found: ' + sets.length + ' sets');

                // Loop through each linked_page entry
                for (let i = 0; i < sets.length; i++) {
                    fs.readdir(path + '/' + sets[i], async (err, pages) => {
                        logger.debug(pages.length + ' pages in set');

                        // Loop through each page of each linked_page entry
                        for (let j = 0; j < pages.length; j++) {
                            // speedup development, TODO REMOVE LATER!
                            /*if(j >= 100) {
                                break;
                            }*/
                            process.stdout.write('Processing page ' + j + ' of ' + pages.length + ' in dataset ' + i + '\r');
                            // Unzip each page and parse the trip URIs of each connection
                            let uncompressedFeed = await utils.readAndGunzip(path + '/' + sets[i] + '/' + pages[j]);
                            ...
                            for (let k = 0; k < jsonldGraph.length; k++) {
                                ...
                                let tripPath = this.storage + '/vehicles/' + companyName + '/' + tripURI.replace('http://', ''); // Remove http from tripURI for path
                                try {
                                ...
                                let stream = fs.createWriteStream(tripPath + '/connections.json', {flags: 'a'});
                                stream.write(graph[k]);
                                stream.end();



                                /*let departureTime = new Date(jsonldGraph[k]['departureTime']);
                                let arrivalTime = new Date(jsonldGraph[k]['arrivalTime']);
                                let canDeparture = jsonldGraph[k]['gtfs:pickupType'] !== 'gtfs:NotAvailable';
                                let canArrival = jsonldGraph[k]['gtfs:dropOffType'] !== 'gtfs:NotAvailable';

                                // Check if entry already exist
                                let existingEntryIndex = this.vehicleIndexData.findIndex((entry) => {
                                    return entry['tripURI'] === tripURI;
                                });

                                // Index > -1, update entry
                                if (existingEntryIndex > -1) {
                                    let entry = this.vehicleIndexData[existingEntryIndex];
                                    let currentDepartureTime = new Date(entry['departureTime']);
                                    let currentArrivalTime = new Date(entry['arrivalTime']);

                                    // Update departure time if new departure time is earlier than the current one.
                                    if (canDeparture && departureTime.getTime() < currentDepartureTime.getTime()) {
                                        entry['departureTime'] = departureTime.toISOString();
                                    }

                                    // Update arrival time if new arrival time is later than the current one.
                                    if (canArrival && arrivalTime.getTime() > currentArrivalTime.getTime()) {
                                        entry['arrivalTime'] = arrivalTime.toISOString();
                                    }

                                    // Update entry in array
                                    this.vehicleIndexData[existingEntryIndex] = entry;

                                    if (tripURI === 'http://irail.be/vehicle/IC530/20190210' && (canArrival || canDeparture)) {
                                        console.debug(entry);
                                    }
                                }
                                // New entry, push it into the array
                                else {
                                    this.vehicleIndexData.push({
                                        'tripURI': tripURI,
                                        'departureTime': departureTime.toISOString(),
                                        'arrivalTime': arrivalTime.toISOString()
                                    })

                                    if (tripURI === 'http://irail.be/vehicle/IC530/20190210') {
                                        console.debug({
                                            'tripURI': tripURI,
                                            'departureTime': departureTime.toISOString(),
                                            'arrivalTime': arrivalTime.toISOString()
                        ...
                        // Finished looping through pages, resolve Promise
                        resolve();
                    });
                }
            });
    ...
    /**
     * Writes the current vehicle index to disk
     * @param dataset
     */
    writeVehicleIndexToDisk(dataset) {
        return new Promise((resolve, reject) => {
            let stream = fs.createWriteStream(this.storage + '/vehicles/' + dataset.companyName + '/index.json')
                        .on('finish', function () {
            ...
            console.log("VEHICLE INDEX=" + this.vehicleIndexData.length);
            stream.write(JSON.stringify(this.vehicleIndexData), () => {
    ...
    generateVehicleIndexTree(dataset) {
        /*
            Generate the a Linked Data Patricia tree to speed up the searching process.
            Based on https://github.com/Dexagod/Delijn-ldf-to-ldtree/blob/master/DeLijnParser.js
        */
        logger.info('Generating vehicle tree...');
        let t0 = new Date().getTime();
        let t1;

        let companyName = dataset.companyName;
        let pTree = new patriciaTree();
        let sourceDir = this.storage + '/vehicles/' + companyName + '/';
        let dataDir = 'data/';
        let treeDir = 'tree/';
        let treeFile = 'vehicleTree';
        let maxCachedFragments = 10000;
        let maxFragmentSize = 50;
        let tree = pTree.createTree(sourceDir, dataDir, maxCachedFragments, maxFragmentSize);

        // Add the data to the tree
        for(let i=0; i < this.vehicleIndexData.length; i++) {
            console.log("Adding entry " + i);
            let leafRepresentation = this.vehicleIndexData[i]['tripURI'];
            let leafValue = {
                'arrivalTime': this.vehicleIndexData[i]['arrivalTime'],
                'departureTime': this.vehicleIndexData[i]['departureTime']
            };
            tree.addData(leafRepresentation, leafValue);
        }
        ...
        pTree.writeTree(tree, treeDir, treeFile)

        // Benchmarking
        t1 = (new Date().getTime() - t0) / 1000;
        logger.info('Vehicle tree generated successfully (took ' + t1 + ' seconds)');
    }
