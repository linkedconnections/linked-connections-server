    ...
    async manage() {
        ...
        this._datasets.forEach(async (dataset, index) => {
            try {
                ...
                await this.sortPagesByTripURI(dataset).then(() => {
                    this.reduceTripURISorting(dataset);
                })
            } catch (err) {
                logger.error(err);
            }
                ...
                for (let i = 0; i < sets.length; i++) {
                    fs.readdir(pagesPath + '/' + sets[i], async (err, pages) => {
                        ...
                        for (let j = 0; j < pages.length; j++) {
                            process.stdout.write('Processing page ' + j + ' of ' + pages.length + ' in data set ' + i + '\r');
                            ...
                            for (let k = 0; k < jsonldGraph.length; k++) {
                                ...
                                // If exists, read the current list of page URIs
                                if(fs.existsSync(tripPath + '/pages.json')) {
                                    fs.readFile(tripPath + '/pages.json', 'utf8', (err, data) => {
                                        currentTrip = JSON.parse(data);
                                    });
                                }
                                else {
                                    currentTrip = null;
                                ...
                                if(!currentTrip) {
                                    currentTrip = {
                                        'departurePage': pages[j],
                                        'arrivalPage': pages[j]
                                    }
                                    let writeStream = fs.createWriteStream(tripPath + '/pages.json', {flags: 'w'});
                                    writeStream.write(JSON.stringify(currentTrip));
                                    writeStream.end();
                                }
                                // Avoid to add double page URI entries
                                else if(currentTrip['arrivalPage'] !== pages[j] && currentTrip['departurePage'] !== pages[j]) {
                                    // Add new page URI to the list and write it to disk
                                    let currentDepartureTime = new Date(url.parse(currentTrip.departurePage, true).query.departureTime);
                                    let currentArrivalTime = new Date(url.parse(currentTrip.arrivalPage, true).query.departureTime);
                                    let newPageTime = new Date(url.parse(pages[j], true).query.departureTime);
                                    console.debug(currentDepartureTime);
                                    
                                    data = {
                                        'departurePage': currentTrip.departurePage,
                                        'arrivalPage': currentTrip.arrivalPage
                                    };
                                    
                                    if(newPageTime < currentDepartureTime) {
                                        data.departurePage = newPageTime;
                                    }
                                    
                                    if(newPageTime > currentArrivalTime) {
                                        data.arrivalPage = newPageTime;
                                    }
                                    
                                    let writeStream = fs.createWriteStream(tripPath + '/pages.json', {flags: 'w'});
                                    writeStream.write(currentTrip);
                                    writeStream.end();
                                }
                            }
                        }
