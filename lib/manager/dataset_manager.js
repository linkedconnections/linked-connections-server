    ...
    processLiveUpdate(index, dataset, rt_path, removeCache) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let tmp = this.storage + '/real_time/' + dataset.companyName + '/.indexes';
                ...
                let trips = await parser.getUpdatedTrips();

                // Get indexes for this specific update
                let lastGtfs = await utils.getLatestGtfsSource(datasets_dir);
                let indexer = new GtfsIndex(lastGtfs, tmp);
                parser.setIndexes(await indexer.getIndexes(trips));
