...
import { PageWriterStream } from './pageWriterStream.js';
...
export class DatasetManager {
    constructor() {
        ...
        this._jobs = [];
        this._indexes = [];
        logger = getLogger(utils.serverConfig.logLevel || 'info');
    }
    ...
    launchStaticJob(index, dataset) {
        const static_job = new CronJob(
            dataset.updatePeriod,
            () => this.processStaticGTFS(index, dataset),
            null,
            true
        );
    ...
    async loadGTFSIdentifiers(index, dataset, tmp) {
        try {
            ...
            const lastGtfs = await utils.getLatestGtfsSource(datasets_dir);

            if (lastGtfs !== null) {
                const rtData = dataset['realTimeData'];
                let indexer = new gtfsrt2lc.GtfsIndex({
                    path: lastGtfs,
                    auxPath: tmp,
    ...
    launchRTJob(index, dataset) {
        const companyName = dataset.companyName;
        const rt_path = this.storage + '/real_time/' + companyName;
        ...
        const rt_job = new CronJob(
            dataset.realTimeData.updatePeriod,
            async () => { await this.processLiveUpdate(index, dataset, rt_path, removeCache) },
            null,
            true
        );
    ...
    processLiveUpdate(index, dataset, rt_path, removeCache) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const fragTimeSpan = dataset.realTimeData.fragmentTimeSpan || 600;
                // Proceed to parse GTFS-RT
                const parser = new gtfsrt2lc.Gtfsrt2LC({
                ...
                const rtlc = await parser.parse({ format: 'jsonld', objectMode: true });
                ...
                rtlc.on('end', async () => {
                    try {
                        ...
                        await Promise.all(rawData.map(async jodata => {
                            ...
                            const ndt = new Date(jodata.departureTime);
                            const newFragment = new Date(ndt.getTime() - (ndt.getTime() % (fragTimeSpan * 1000)));
                            // Determine connection's original fragment
                            const odt = new Date(ndt.getTime() - (jodata['departureDelay'] * 1000));
                            const origFragment = new Date(odt.getTime() - (odt.getTime() % (fragTimeSpan * 1000)));
                            ...
                            if (fs.existsSync(rt_path + '/' + origFragment.toISOString() + '.remove')) {
                                ...
                                if (removeCache[origFragment.toISOString()]) {
                                    ...
                                    const removeFile = fs.readFileSync(rt_path + '/' + origFragment.toISOString() + '.remove', 'utf8').split('\n');
                                    removeFile.pop();
                                    origRemove = new Map();
    ...
    rtCompressionJob(dataset) {
        ...
        new CronJob(
            dataset.realTimeData.compressionPeriod,
            async () => {
                try {
                    const path = this.storage + '/real_time/' + companyName + '/';
                    const now = new Date();
                    const fgmts = await readdir(path);

                    fgmts.forEach(fg => {
                        if (fg.indexOf('.gz') < 0) {
                            // Get last modification time of the file
                            const stats = fs.statSync(path + '/' + fg);
                            const lastModified = new Date(util.inspect(stats.mtime));
    ...
    cleanRemoveCache(removeCache, timestamp) {
        ...
        const timeLimit = new Date(timestamp.getTime() - (3 * 3600 * 1000));
        const keys = Object.keys(removeCache);
        for (let i in keys) {
    ...
    storeRemoveList(removeList, path, memento) {
        Object.entries(removeList).forEach(async ([key, value]) => {
            const file_path = path + '/' + key + '.remove';
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                Object.entries(data).forEach(async ([key, value]) => {
                    const updData = value.join('\n');
                    const file_path = path + '/' + key + '.jsonld';
    ...
    cleanUpIncompletes() {
        return Promise.all(this._datasets.map(async dataset => {
            if (fs.existsSync(this.storage + '/datasets/' + dataset.companyName)) {
                const files = await readdir(this.storage + '/datasets/' + dataset.companyName);
                let incomplete = null;
