...
class DatasetManager {
    ...
    async manage() {
        ...
        this._datasets.forEach(async (dataset, index) => {
            try {
                ...
                // TEST vehicle index
                await this.updateVehicleIndex(dataset);
                            ...
                            reader.pipe(new jsonldstream.Deserializer())
                                ...
                                .on('finish', async () => {
                                    ...
                                    comp.on('close', async (code) => {
                                        ...
                                        // Connections are now available in Linked Connections format, trigger a vehicle index generation.
                                        await this.updateVehicleIndex(dataset);
                                    });
                                });
                        });
     ...
     */
    async updateVehicleIndex(dataset) {
            ...
            for(let i=0; i < sets.length; i++) {
                fs.readdir(path + '/' + sets[i], async(err, pages) =>
                {
                    ...
                    for(let j=0; j < pages.length; j++) {
                        ...
                        let uncompressed_feed = await utils.readAndGunzip(path + '/' + sets[i] + '/' + pages[j]);
                        let jsonld_graph = uncompressed_feed.split(',\n').map(JSON.parse);

                        for(let k=0; k < jsonld_graph.length; k++) {
                            let tripURI = jsonld_graph[k]['gtfs:trip'];
                            let departureTime = new Date(jsonld_graph[k]['departureTime']);
                            let arrivalTime = new Date(jsonld_graph[k]['arrivalTime']);
                            let canDeparture = jsonld_graph[k]['gtfs:pickupType'] !== 'gtfs:NotAvailable';
                            let canArrival = jsonld_graph[k]['gtfs:dropOffType'] !== 'gtfs:NotAvailable';

                            // Check if entry already exist
                            let existingEntryIndex = data.findIndex((entry) => {
                                return entry['tripURI'] === tripURI;
                            });

                            // Index > -1, update entry
                            if(existingEntryIndex > -1) {
                                let entry = data[existingEntryIndex];
                                //console.debug('Current TRIP: ' + tripURI);
                                //console.debug('Matching with: ' + entry['tripURI']);
        ...
        pTree.createTree(sourceDir, dataDir, maxCachedFragments, maxFragmentSize);
        ...
        /*if(file_name != null) {
            ...
            let uncompressed_feed = await utils.readAndUnzip(path);
            let jsonld_graph = uncompressed_feed.split(',\n')//.map(JSON.parse);
            console.debug("First element of graph=" + jsonld_graph);

            t1 = (new Date().getTime() - t0) / 1000;
            logger.info('Vehicle index updated successfully (took ' + t1 + ' seconds)');
        }
        else {
            logger.warn('Vehicle index already created for dataset');
        }*/
    }
