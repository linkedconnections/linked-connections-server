...
class DatasetManager {
                            ...
                            reader.pipe(new jsonldstream.Deserializer())
                                ...
                                .on('finish', async () => {
                                    ...
                                    comp.on('close', async (code) => {
                                        ...
                                        // TEST cancel trip
                                        this.cancelTrip('http://irail.be/vehicle/IC106/20190226', companyName);
                                    });
                                });
                        });
                });
    ...
    async cancelTrip(tripURI, companyName, startingConnection = null) {
        ...
        while(arrivalPageTime.getTime() >= currentPageTime) {
            ...
            let connectionDepartureTime = new Date(new Date(foundCancelledConnection['departureTime']).getTime() - foundCancelledConnection['departureDelay']);
            ...
            if(foundRTFragment == null) {
                let rtTimestamp = connectionDepartureTime; // THIS NEEDS TO BE IMPROVED! 
                foundRTFragment = rtPath + '/' + rtTimestamp + '.jsonld';
            }
