...
class DatasetManager {
    ...
    async processStaticGTFS(index, dataset) {
        ...
        try {
            ...
            if (file_name != null) {
                ...
                let converter = new Connections({ format: 'jsonld', store: 'MemStore', baseUris: baseURIs });
                // Path where connections will be created
                let connsPath = this.storage + '/linked_connections/' + companyName + '/';
                ...
                converter.resultStream(uncompressed_feed, connsPath, async rawConns => {
                    ...
                    let sorted_path = this.storage + '/linked_connections/' + companyName + '/' + file_name + '.json';
                    await this.sortLCByDepartureTime(rawConns, sorted_path);
                    // Delete unsorted Linked Connections graph
                    await del([rawConns], { force: true });
                    ...
                    reader.pipe(new jsonldstream.Deserializer())
                        ...
                        .on('finish', async () => {
                            ...
                            await this.compressAll(file_name, companyName);
                            let t1 = (new Date().getTime() - t0) / 1000;
                            logger.info('Dataset conversion for ' + companyName + ' completed successfully (took ' + t1 + ' seconds)');
                            // Delete .lock file
                            await del([this.storage + '/datasets/' + companyName + '/' + file_name + '.lock'], { force: true });
                            // Update Catalog, Stops and Routes
                            let [stops, routes] = await Promise.all([
                            ...
                            await del([uncompressed_feed], { force: true });
                        });
                });
            } else {
            ...
            try {
                ...
                rtlc.on('end', async () => {
                    try {
                        ...
                        // Archive latest.json if it exists, then create a new latest.json with new updates
                        if (fs.existsSync(`${this.storage}/feed_history/${dataset['companyName']}/latest.json`)) {
                            const now = new Date();
                            fs.renameSync(`${this.storage}/feed_history/${dataset['companyName']}/latest.json`, `${this.storage}/feed_history/${dataset['companyName']}/${now.toISOString()}.json`);
                        }
    ...
    sortLCByDepartureTime(unsorted, sorted) {
        return new Promise(async (resolve, reject) => {
            ...
            let conn = (await exec(`head -2 ${unsorted} | tail -1`))['stdout'];
    ...
    compressAll(file_name, companyName) {
        return new Promise((resolve, reject) => {
            try {
                let count = 0;
                let done = () => {
                ...
                // Compress complete Connections file
                child_process.spawn('gzip', [`${file_name}.json`], {
                    cwd: this.storage + '/linked_connections/' + companyName,
                    stdio: 'ignore'
                }).on('close', () => done());

                // Compress all Connection fragments
                child_process.spawn('find', ['.', '-type', 'f', '-exec', 'gzip', '{}', '+'], {
        ...
        return Promise.all(this._datasets.map(async dataset => {
            if (fs.existsSync(this.storage + '/datasets/' + dataset.companyName)) {
                ...
                if (incomplete !== null) {
                    ...
                    this.storage + '/linked_connections/' + dataset.companyName + '/.routes',
                    this.storage + '/linked_connections/' + dataset.companyName + '/.trips',
                    this.storage + '/linked_connections/' + dataset.companyName + '/.stops',
                    this.storage + '/linked_connections/' + dataset.companyName + '/.services',
