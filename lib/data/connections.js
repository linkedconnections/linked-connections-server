...
class Connections {
    ...
    async init() {
        try {
            ...
            this.clear();
            // Create AVL Tree covering the whole dataset as it is only one day
            const now = new Date();
            ...
            const fragments = this.staticData['staticFragments'][this.agency][versions[0]]
            this.min = fragments[0];
            this.max = fragments[fragments.length - 1];
            const path = `${utils.datasetsConfig['storage']}/linked_pages/${this.agency}/${versions[0]}`;
            let connCount = 0;
            ...
            let recreateTree = new cron.CronJob({
                cronTime: '0 0 3 * * *', // TODO: make this configurable
                onTick: () => {
