...
class Connections {
    ...
    async init() {
        try {
            ...
            this.clear();
            // Create AVL Tree from now until 50 fragments in the future
            const now = new Date();
            ...
            // Setup a cron job to recreate the tree every 10 minutes
            let recreateTree = new cron.CronJob({
                cronTime: '0 */10 * * * *', // TODO: make configurable
                onTick: () => {
        ...
        try {
            ...
            await Promise.all(newConns.map(async conn => {
                ...
                if (curr >= this.min && curr < this.max) {
                    ...
                    // Check if the connection changed and update it in the tree
                    if (curr !== dep) {
                        this.avlTree.remove(dep, '@id', conn['@id']);
                        this.avlTree.insert(curr, conn);
                        this.delayIndex[conn['@id']] = conn['departureDelay'];
                    } else if (!this.delayIndex[conn['@id']] && this.delayIndex[conn['@id']] !== 0) {
                        // Departure time didn't change this connection was not reported before
