/**
 * This code is based on https://github.com/w8r/avl
 */

const DEFAULT_COMPARE = (a, b) => { return a > b ? 1 : a < b ? -1 : 0; };
...
class AVLTree {
    ...
    find(key, param, value) {
        ...
        while (subtree) {
            ...
            if (cmp === 0) {
                ...
                if (param && value && subtree.data[param] !== value) {
                    // Get previous and next nodes
                    let prev = this.prev(subtree);
                    let next = this.next(subtree);
                    while ((next && compare(key, next.key) === 0) || (prev && compare(key, prev.key) === 0)) {
                        // Check internal values
                        if(next && next.data[param] === value) {
                            return next;
                        }
                        if(prev && prev.data[param] === value) {
                            return prev;
                        }
     ...
     * @return {?Node}
     */
    remove(key, node, param, value) {
        if (!this._root) return null;

        var compare = this._comparator;
        var cmp = 0;

        var node = node ? node : this.find(key, param, value);
        if (!node) return null;
