...
class Feed {
    ...
    async getFeed(req, res) {
        ...
        try {
            ...
            // sort members on departure time so we can extract the min and max date. These dates will be
            // used for the relations in the LDES
            members.sort((a, b) => {
            ...
            // get max departureTime in fragment (used as tree:value for the GreaterThanRelation)
            const max_dt = members[members.length - 1].departureTime;
            ...
            members.forEach((member) => {
                ...
                member["@id"] += (member["@id"].slice(-1) === "/") ? `${created.toISOString()}` : `/${created.toISOString()}`;
            ...
            const params = {
                ...
                maxDeparture: max_dt    // ISO8601 string of latest departureTime in fragment
            };
