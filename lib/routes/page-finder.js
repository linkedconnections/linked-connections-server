...
class PageFinder {
    ...
    async getConnections(req, res) {
        ...
        const v0 = Object.keys(staticData.staticFragments[agency])[0];
        // Redirect to first fragment for the experiment if no date is given
        // or date is older than first fragment
        if (departureTime.toString() === 'Invalid Date') {
            departureTime = new Date(staticData.staticFragments[agency][v0][0]);
        } else if (departureTime.getTime() < staticData.staticFragments[agency][v0][0]) {
            departureTime = new Date(staticData.staticFragments[agency][v0][0]);
        }
        ...
        try {
            ...
            if (departureTime.getTime() >= connTrees[agency].min && departureTime.getTime() < connTrees[agency].max) {
                ...
                if (conf['realTimeData']) {
                    ...
                    res.set({
                        'Cache-Control': 'public, s-maxage=' + maxage + ', max-age=' + (maxage + 1)
