...
class PageFinder {
    ...
    async getConnections(req, res) {
        ...
        // Redirect to first fragment for the experiment of no date is given
        if (departureTime.toString() === 'Invalid Date') {
            const key = Object.keys(staticData.staticFragments[agency])[0];
            departureTime = new Date(staticData.staticFragments[agency][key][0]);
        }
