...
class Memento {
    async getMemento(req, res) {
        ...
        try {
            ...
            if (rt_exists) {
                if (utils.handleConditionalGET(req, res, rtfs[rtfs.length - 1], true, departureTime, mementoDate)) {
                    return;
                }
            } else {
                if (utils.handleConditionalGET(req, res, sf_path, false, departureTime, mementoDate)) {
                    return;
                }
            }
            ...
            res.set(headers);
            res.json(await utils.addHydraMetada(params));
