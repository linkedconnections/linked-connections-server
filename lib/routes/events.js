...
class Events {
    ...
    async _addListenerPubSub(req, res, lastSyncTime, agency) {
        ...
        while(syncTime.getTime() < now.getTime()) {
            missingPage = await this._getEventsPage(req, res, syncTime, agency); 
            this._pushToClient(res, lastSyncTime.toISOString(), JSON.stringify(missingPage));
            if('hydra:next' in missingPage) {
                syncTime = new Date(url.parse(missingPage['hydra:next'], true).query['lastSyncTime']);
    ...
    async _handlePubSub(agency, path) {
        ...
        this.listeners[agency].forEach((client) => {
          this._pushToClient(client, agency, data);
        });
    }
