...
class Events {
    constructor() {
        ...
        this._watcher = chokidar.watch(this.eventsStorage, { ignored: /(^|[\/\\])\../, ignoreInitial: true }).on('add', (file, info) => {
                            ...
                            if(this.lastPubSubTime.getTime() < eventsFileDate.getTime()) {
                                ...
                                this.logger.debug(`Found a new events file: ${eventsFileDate.toISOString()}.jsonld, publishing to SSE clients...`);
                                let agency = path.basename(path.dirname(file));
                                this.handlePubSub(agency, file);
                            }
                        });
    }
    ...
    addListenerPubSub(req, res, lastSyncTime, agency) {
        ...
        this._pushToClient(res, `init`, `INIT`);
    }

    handlePubSub(agency, path) {
        // Read events TODO

        // Publish SSE events for all listeners
        this.listeners[agency].forEach((client) => {
          this._pushToClient(client, agency, `TEST`);
        });
    }
    ...
    async getEventsPage(req, res, when, agency) {
        ...
        for(let e=0; e < entries.length; e++) {
            ...
            if(eventsDateCurrent.getTime() <= when.getTime() && (eventsDateNext == null? true: when.getTime() < eventsDateNext.getTime())) {
                ...
                page = this.addHydraMetaData(req, res, page, agency, eventsDatePrevious, eventsDateCurrent, eventsDateNext);
                return page;
            }
        }
