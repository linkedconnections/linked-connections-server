...
class Events {
	...
	this._pushJob = new cron.CronJob({
		cronTime: '0 0 3 * * *',
		onTick: async () => {
			console.error('Polling file system for new events...');
	...
	fs.readdir(this.eventsStorage, async (err, files) => {
    ...
    async _handlePubSub(agency, file) {
        ...
        this.listeners[agency].forEach((client) => {
            this._pushToClient(client, currentPageTimestamp, page);
        });
    }
	...
	console.error(JSON.stringify(dataCloned))
